...
const Header: React.FC<{ title: string }> = ({ title }) => {
  ...
  return (
    <div className="flex justify-between items-center bg-inaoe p-3">
      <button
      ...
      <div className="relative">
        ...
        {showDropdown && (
          <div className="absolute top-10 right-0 bg-white shadow-md rounded-md py-2 px-4 z-10 w-55">
            <button
              className="flex w-full text-left py-1 px-2 items-center"
              onClick={handleLogout}
            >
              <img src={logoutIcon} alt="logout-icon" className="w-4 mr-2" />
              Cerrar Sesión
            </button>
          </div>
        )}
